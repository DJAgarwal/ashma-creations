...
@section('admin_title', 'Create Collection')
@section('admin_page_header', 'Create Collection')
...
@section('admin_content')
    ...
    <div class="max-w-2xl bg-white rounded-[2rem] p-8 shadow-sm border border-primary-light/10">
        <form method="POST" action="{{ route('admin.collections.store') }}" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @include('pages.admin.collections._form', ['item' => null])
            <button type="submit" class="w-full py-4 bg-primary text-white font-bold rounded-2xl shadow-lg shadow-primary/20 hover:bg-accent transition-all cursor-pointer">Save Collection</button>
        </form>
    </div>
@endsection
