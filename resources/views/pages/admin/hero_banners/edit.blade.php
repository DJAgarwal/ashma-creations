...
@section('admin_title', 'Edit Banner')
@section('admin_page_header', 'Edit Banner')
    ...
    <div class="max-w-4xl mx-auto space-y-8">
        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <h3 class="text-2xl font-bold text-gray-800">Edit Banner #{{ $heroBanner->id }}</h3>
            </div>
            <div>
                @if ($heroBanner->trashed())
                    <span class="px-3 py-1 bg-red-100 text-red-700 rounded-full text-xs font-bold">Trashed (Soft Deleted)</span>
                @endif
            </div>
        </div>
        ...
        <form method="POST" action="{{ route('admin.homepage.hero-banners.update', $heroBanner->id) }}" enctype="multipart/form-data" class="space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white p-6 md:p-8 rounded-[2rem] shadow-sm border border-primary-light/10 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-soft-gray mb-3">Leave blank to keep current image (Recommended 1920 × 700 px)</p>
                        <input type="file" id="desktop_image" name="desktop_image" accept="image/*"
                        ...
                        <!-- Current / Preview Desktop Image -->
                        <div class="mt-4 border border-primary-light/30 rounded-2xl p-2 bg-background flex flex-col items-center justify-center min-h-[140px]">
                            @if ($heroBanner->desktop_image)
                                <img id="desktop-preview" src="{{ asset($heroBanner->desktop_image) }}" alt="Desktop Preview" class="max-h-40 w-full object-cover rounded-xl">
                            @else
                                <img id="desktop-preview" src="#" alt="Desktop Preview" class="max-h-40 w-full object-cover rounded-xl hidden">
                                <span id="desktop-placeholder" class="text-xs text-soft-gray">No desktop image present</span>
                            @endif
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <p class="text-xs text-soft-gray mb-3">Leave blank to keep current image (Recommended 800 × 1000 px)</p>
                        <input type="file" id="mobile_image" name="mobile_image" accept="image/*"
                               class="w-full text-xs text-gray-500 file:mr-4 file:py-2.5 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-primary/10 file:text-primary hover:file:bg-primary hover:file:text-white cursor-pointer transition-all">
                        
                        <!-- Current / Preview Mobile Image -->
                        <div class="mt-4 border border-primary-light/30 rounded-2xl p-2 bg-background flex flex-col items-center justify-center min-h-[140px]">
                            @if ($heroBanner->mobile_image)
                                <img id="mobile-preview" src="{{ asset($heroBanner->mobile_image) }}" alt="Mobile Preview" class="max-h-40 object-contain rounded-xl">
                                <label class="mt-2 text-xs font-semibold text-red-600 flex items-center gap-1 cursor-pointer">
                            ...
                            @else
                                ...
                                <span id="mobile-placeholder" class="text-xs text-soft-gray text-center">No mobile image uploaded<br>(desktop image used)</span>
                            @endif
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select id="link_type" name="link_type" class="w-full px-4 py-3 bg-background border border-primary-light/20 rounded-xl text-sm focus:outline-none focus:border-primary text-gray-800 cursor-pointer">
                            @foreach (['Category', 'Collection', 'Occasion', 'Recipient', 'Product', 'Page', 'Custom URL'] as $type)
                                <option value="{{ $type }}" {{ old('link_type', $heroBanner->link_type) === $type ? 'selected' : '' }}>
                                    {{ $type }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <div id="target-Category" class="target-container">
                            ...
                            <select id="link_id_category" class="link-id-input w-full px-4 py-3 bg-background border border-primary-light/20 rounded-xl text-sm focus:outline-none focus:border-primary text-gray-800 cursor-pointer">
                                ...
                                @foreach ($destinations['categories'] as $item)
                                    <option value="{{ $item->id }}" {{ old('link_id', $heroBanner->link_id) == $item->id ? 'selected' : '' }}>
                                        {{ $item->name }} ({{ $item->slug }})
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div id="target-CustomURL" class="target-container hidden">
                            ...
                            <input type="text" id="custom_url" name="custom_url" value="{{ old('custom_url', $heroBanner->custom_url) }}" 
                        ...
                        <input type="hidden" id="final_link_id" name="link_id" value="{{ old('link_id', $heroBanner->link_id) }}">
                    </div>
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
                    ...
                    <div>
                        ...
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="active" value="1" {{ old('active', $heroBanner->active) ? 'checked' : '' }} class="sr-only peer">
                    ...
                    <div>
                        ...
                        <input type="number" id="display_order" name="display_order" value="{{ old('display_order', $heroBanner->display_order) }}" min="0" 
